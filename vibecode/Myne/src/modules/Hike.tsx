...
const DIFF_CONFIG: Record<HikeDifficulty, { label: string; color: string; bg: string; dot: string }> = {
  easy:     { label: 'Easy',     color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-900/40', dot: '#10b981' },
  moderate: { label: 'Moderate', color: 'text-amber-400',   bg: 'bg-amber-500/10 border-amber-900/40',     dot: '#f59e0b' },
  hard:     { label: 'Hard',     color: 'text-orange-400',  bg: 'bg-orange-500/10 border-orange-900/40',   dot: '#f97316' },
...
export default function Hike() {
  ...
  const wishFormFields = (form: typeof EMPTY_WISH_FORM, set: React.Dispatch<React.SetStateAction<typeof EMPTY_WISH_FORM>>) => (
    <div className="space-y-3">
      <div>
        ...
        <input className={INPUT} placeholder="Hike name" value={form.name} onChange={e => set(f => ({ ...f, name: e.target.value }))} autoFocus />
      </div>
      <div>
        <label className={LABEL}>Location *</label>
        <input className={INPUT} placeholder="Location" value={form.location} onChange={e => set(f => ({ ...f, location: e.target.value }))} />
      </div>
  ...
  const TABS = [
    ...
    { id: 'ranking'  as Tab, label: 'Ranking',    count: null },
    { id: 'gear'     as Tab, label: 'Gear',       count: null },
  ];
  ...
  return (
    <div style={{ background: '#09090f', minHeight: '100%' }}>
      ...
      <div className="relative overflow-hidden px-6 pt-8 pb-6">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {done.length} hike{done.length !== 1 ? 's' : ''} done · {wishlist.length} in wishlist
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Hikes',       val: String(done.length),                                     unit: '',   icon: '🥾' },
              { label: 'Distance',    val: totalDistance > 0 ? totalDistance.toFixed(1) : '—',      unit: 'km', icon: '📏' },
              { label: 'Elevation',   val: totalElevation > 0 ? totalElevation.toLocaleString('en-GB') : '—', unit: 'm', icon: '⛰️' },
              { label: 'Avg. rating', val: done.length > 0 ? String(avgRating) : '—',              unit: done.length > 0 ? '★' : '', icon: '🏆' },
            ].map(s => (
          ...
          <div>
            ...
            {showAddWish && !editWish && (
              <div style={{ ...GLASS, padding: 20, marginBottom: 16 }} className="space-y-3">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">New hike</p>
                {wishFormFields(wishForm, setWishForm)}
                <div className="flex gap-2 justify-end pt-1">
                  <button onClick={() => setShowAddWish(false)} className={BTN_GHOST}>Cancel</button>
                  <button onClick={addWish} className={BTN_PRIMARY}>Add</button>
                </div>
              </div>
            )}
            ...
            {editWish && (
              <div style={{ ...GLASS, padding: 20, marginBottom: 16, border: `1px solid ${accent}50` }} className="space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: accent }}>Edit</p>
                {wishFormFields(wishForm, setWishForm)}
                <div className="flex gap-2 justify-end pt-1">
                  <button onClick={() => setEditWish(null)} className={BTN_GHOST}>Cancel</button>
                  <button onClick={submitEditWish} className={BTN_PRIMARY}>Save</button>
                </div>
              </div>
            )}
            ...
            {filteredWish.length === 0 ? (
              <div className="text-center py-16" style={GLASS}>
                ...
                <p className="text-gray-400 font-medium mb-1">
                  {wishFilter !== 'all' ? `No "${DIFF_CONFIG[wishFilter].label}" hikes` : 'Your wishlist is empty'}
                </p>
                <p className="text-gray-600 text-sm">
                  {wishFilter !== 'all' ? 'Try another filter' : 'Add your next adventures!'}
                </p>
              </div>
            ) : (
            ...
            <div
              ...
              {dropActive ? '⬇ Drop here to mark as done' : 'Drag a card here to mark it as done'}
            </div>
          </div>
        )}
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-4">
              <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'rgba(255,255,255,0.05)' }}>
                {([{ v: 'date', l: '📅 Date' }, { v: 'rating', l: '⭐ Rating' }] as const).map(s => (
            ...
            {done.length === 0 ? (
              <div className="text-center py-16" style={GLASS}>
                ...
                <p className="text-gray-400 font-medium mb-1">No hikes done yet</p>
                <p className="text-gray-600 text-sm">Your adventures will appear here!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {sortedDone.map(d => (
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {new Date(d.date + 'T00:00').toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                      </div>
        ...
        {tab === 'ranking' && (
          <div>
            {done.length === 0 ? (
              <div className="text-center py-16" style={GLASS}>
                <p className="text-5xl mb-3">🏆</p>
                <p className="text-gray-400 font-medium mb-1">No ranking yet</p>
                <p className="text-gray-600 text-sm">Rate your hikes to build your top!</p>
              </div>
            ) : (
              <>
                      ...
                      <div key={h.id} className="flex items-center gap-4 transition-all hover:brightness-110"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 mt-0.5">
                            ...
                            {new Date(h.date + 'T00:00').toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </p>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div style={GLASS} className="p-5">
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-semibold text-white text-sm">🎒 Items to pack</h3>
              ...
              {gearCheck.length > 0 && (
                <div className="mb-4">
                  <div className="flex items-center justify-between text-xs text-gray-500 mb-1.5">
                    <span>{gearCheck.filter(g => g.checked).length}/{gearCheck.length} packed</span>
              ...
              {gearCheck.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-600">No items — add your gear</p>
                </div>
              ) : (
            ...
            <div style={GLASS} className="p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-white text-sm flex items-center gap-1.5">
                  <ShoppingBag size={14} style={{ color: accent }} /> Gear to buy
                </h3>
              ...
              {showAddShop && !editShop && (
                <div className="rounded-xl p-3 mb-3 space-y-2" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <input className={INPUT} placeholder="Item name *" value={shopForm.title} onChange={e => setShopForm(f => ({ ...f, title: e.target.value }))} autoFocus />
                  <input className={INPUT} placeholder="Shop link (optional)" value={shopForm.link} onChange={e => setShopForm(f => ({ ...f, link: e.target.value }))} />
                  <input type="number" step="0.01" className={INPUT} placeholder="Price (€, optional)" value={shopForm.price} onChange={e => setShopForm(f => ({ ...f, price: e.target.value }))} />
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setShowAddShop(false)} className={BTN_GHOST}>Cancel</button>
                    <button onClick={addShop} className={BTN_PRIMARY}>Add</button>
                  </div>
                </div>
              )}
              ...
              {editShop && (
                <div className="rounded-xl p-3 mb-3 space-y-2" style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${accent}40` }}>
                  <p className="text-xs font-semibold" style={{ color: accent }}>Edit</p>
                  <input className={INPUT} value={shopForm.title} onChange={e => setShopForm(f => ({ ...f, title: e.target.value }))} autoFocus />
                  <input className={INPUT} placeholder="Shop link" value={shopForm.link} onChange={e => setShopForm(f => ({ ...f, link: e.target.value }))} />
                  <input type="number" step="0.01" className={INPUT} placeholder="Price (€)" value={shopForm.price} onChange={e => setShopForm(f => ({ ...f, price: e.target.value }))} />
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setEditShop(null)} className={BTN_GHOST}>Cancel</button>
                    <button onClick={submitEditShop} className={BTN_PRIMARY}>Save</button>
                  </div>
                </div>
              )}
              ...
              {gearShop.length === 0 && !showAddShop && !editShop ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-600">Empty list — add your gear</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {gearShop.map(g => (
      ...
      {/* ── Done modal ── */}
      <Modal isOpen={doneModal} onClose={() => { setDoneModal(false); setEditDoneTarget(null); }} title={editDoneTarget ? 'Edit hike' : 'Completed hike 🥾'}>
        <div className="space-y-4">
          <div>
            <label className={LABEL}>Name *</label>
            <input className={INPUT} placeholder="Hike name" value={doneForm.name} onChange={e => setDoneForm(f => ({ ...f, name: e.target.value }))} autoFocus />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div>
            ...
            <textarea className={INPUT} rows={2} placeholder="Impressions, weather conditions..." value={doneForm.comment} onChange={e => setDoneForm(f => ({ ...f, comment: e.target.value }))} />
          </div>
          <div className="flex gap-2 justify-end pt-1">
            <button onClick={() => { setDoneModal(false); setEditDoneTarget(null); }} className={BTN_GHOST}>Cancel</button>
