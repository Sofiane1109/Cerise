...
const METRICS: {
  ...
  { key: 'protein',  label: 'Protéines', unit: 'g',    icon: <Zap size={20} />,            color: 'bg-blue-500',   barColor: '#3b82f6', glow: 'rgba(59,130,246,0.3)'  },
  { key: 'water',    label: 'Eau',       unit: 'ml',   icon: <Droplets size={20} />,       color: 'bg-cyan-500',   barColor: '#06b6d4', glow: 'rgba(6,182,212,0.3)'   },
  { key: 'creatine', label: 'Créatine',  unit: 'g',    icon: <Apple size={20} />,          color: 'bg-purple-500', barColor: '#a855f7', glow: 'rgba(168,85,247,0.3)'  },
];
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">{new Date().toLocaleDateString('fr-FR', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          </div>
        </div>
      ...
      <div className="p-5" style={GLASS}>
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-white">
            Saisir {isToday ? "aujourd'hui" : parseLocal(logDate).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
          </h2>
            ...
            {[...logs].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 14).map(l => (
              <button key={l.date} onClick={() => setLogDate(l.date)}
                ...
                <span className="text-gray-400">
                  {parseLocal(l.date).toLocaleDateString('fr-FR', { weekday: 'short', month: 'short', day: 'numeric' })}
                </span>
                <div className="flex gap-4 text-xs flex-wrap justify-end">
                  ...
                  <span className="text-cyan-400 font-medium">{l.water}ml eau</span>
                </div>
              </button>
            ))}
          </div>
