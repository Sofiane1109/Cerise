...
import { Plus, Trash2, Clock, Tag, AlertCircle, GripVertical } from 'lucide-react';
        ...
        {columnData.map(col => (
          <div
            ...
            className={`flex flex-col rounded-xl border-t-2 ${col.accent} bg-gray-900/50 min-h-96 transition-all ${
              dropTarget === col.status && dragId ? 'bg-gray-800/80 ring-1 ring-inset ring-indigo-500/20' : ''
            }`}
          >
            {/* Column header */}
            ...
            <div className="flex-1 p-3 space-y-2">
              {col.tasks.length === 0 && (
                <div className={`border-2 border-dashed rounded-lg py-10 text-center text-xs transition-colors ${
                  dropTarget === col.status && dragId ? 'border-indigo-500/50 text-indigo-400' : 'border-gray-800 text-gray-700'
                }`}>
              ...
              {col.tasks.map(t => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-grab active:cursor-grabbing select-none transition-all ${
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <GripVertical size={14} className="text-gray-600 shrink-0 mt-0.5" />
                      ...
                      <button
                        onClick={() => remove(t.id)}
                        className="shrink-0 w-6 h-6 flex items-center justify-center rounded text-gray-700 hover:text-red-400 transition-colors"
                      >
                        <Trash2 size={12} />
                      </button>
                    </div>
